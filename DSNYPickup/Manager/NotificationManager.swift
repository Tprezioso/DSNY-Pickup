...
final class NotificationManager: ObservableObject {
    ...
    func createLocalNotification(id: String, day: EnumDays, hour: Int, minute: Int) async {
    ...
    func updateLocalNotification(id: String, day: EnumDays, hour: Int, minute: Int) async {
        let notifications = await UNUserNotificationCenter.current().pendingNotificationRequests()
        if notifications.isEmpty {
            await self.createLocalNotification(id: id, day: day, hour: hour, minute: minute)
        } else {
            for request in notifications {
                if request.identifier == id {
                    UNUserNotificationCenter.current().removePendingNotificationRequests(withIdentifiers: [request.identifier])
                    await self.createLocalNotification(id: id, day: day
